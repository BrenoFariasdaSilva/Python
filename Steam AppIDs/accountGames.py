...
def download_xml(steam_account):
    ...
    if response.status_code == 200:  # If the request was successful (status code 200 - OK)
        ...
        raise Exception(
            f"{BackgroundColors.RED}Failed to {BackgroundColors.CYAN}{steam_account}{BackgroundColors.RED} download XML. Status code: {BackgroundColors.YELLOW}{response.status_code}{Style.RESET_ALL}"
        )  # Raise an exception
...
def get_game_list(xml_content):
    ...
    for game in games:  # For each game in the list of games
        ...
        if app_name_element is not None and app_id_element is not None:
            ...
            if hours_played_element is not None:  # If the hours played element is not None, get the hours played
                ...
                game_info.append(
                    {"appName": app_name, "appID": app_id, "hoursPlayed": hours_played}
    ...
    if total_games != len(
        game_info
    ):  # If the number of total games and the length of the game info list do not match, print a warning
        print(
            f"{BackgroundColors.RED}Warning: Total Games {BackgroundColors.CYAN}({total_games}){BackgroundColors.RED} and Gathered Games {BackgroundColors.CYAN}({len(game_info)}){BackgroundColors.RED} do not match!{Style.RESET_ALL}"
        )
...
def main():
    print(
        f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Steam AppIDs{BackgroundColors.GREEN} program!{Style.RESET_ALL}\n"
    ...
    for steam_account in STEAM_ACCOUNTS:  # For each Steam account in the list of Steam accounts
        ...
        try:  # Try to download the XML file and create the CSV file
            ...
            print(
                f"{BackgroundColors.GREEN}Downloaded and created {BackgroundColors.CYAN}{csv_filename}{BackgroundColors.GREEN} with {BackgroundColors.CYAN}{len(games_list)}{BackgroundColors.GREEN} games.{Style.RESET_ALL}"
            )  # Output the success message
        except Exception as e:  # If an exception occurs
            print(
                f"{BackgroundColors.RED}Error: {BackgroundColors.CYAN}{e}{Style.RESET_ALL}"
