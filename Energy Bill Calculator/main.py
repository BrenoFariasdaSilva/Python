...
def calcular_itens_variaveis(consumo_kwh: float, tarifas: dict, taxas_fixas: dict) -> dict:
   ...
   subtotal = energia_consumo + uso_sistema + energia_bv + energia_bv_p2 + taxas_fixas["bonus_itaipu"] + taxas_fixas["iluminacao_publica"]

   return {"energia_consumo": energia_consumo, "uso_sistema": uso_sistema, "energia_bv": energia_bv, "energia_bv_p2": energia_bv_p2, "subtotal": subtotal}
...
def exibir_resultados(leitura_anterior: float, leitura_atual: float, consumo_kwh: float, itens: dict, tributos: dict, total: float) -> None:
   """
   ...
   print(f"- {BackgroundColors.GREEN}Energia Elétrica Consumo:{Style.RESET_ALL}     {BackgroundColors.CYAN}R$ {itens['energia_consumo']:.2f}{Style.RESET_ALL}")
   print(f"- {BackgroundColors.GREEN}Uso do Sistema de Distrib.:{Style.RESET_ALL}   {BackgroundColors.CYAN}R$ {itens['uso_sistema']:.2f}{Style.RESET_ALL}")
   print(f"- {BackgroundColors.RED}Bandeira Vermelha (P1):{Style.RESET_ALL}       {BackgroundColors.CYAN}R$ {itens['energia_bv']:.2f}{Style.RESET_ALL}")
   print(f"- {BackgroundColors.RED}Bandeira Vermelha (P2):{Style.RESET_ALL}       {BackgroundColors.CYAN}R$ {itens['energia_bv_p2']:.2f}{Style.RESET_ALL}")
   print(f"- {BackgroundColors.YELLOW}Bônus Itaipu:{Style.RESET_ALL}                 {BackgroundColors.CYAN}R$ {-21.65:.2f}{Style.RESET_ALL}")
   print(f"- {BackgroundColors.YELLOW}Iluminação Pública:{Style.RESET_ALL}           {BackgroundColors.CYAN}R$ {37.54:.2f}{Style.RESET_ALL}\n")

   print(f"{BackgroundColors.GREEN}--Tributos:{Style.RESET_ALL}")
   print(f"- {BackgroundColors.RED}ICMS (19.00%):{Style.RESET_ALL}                {BackgroundColors.CYAN}R$ {tributos['icms']:.2f}{Style.RESET_ALL}")
   print(f"- {BackgroundColors.RED}COFINS (5.3936%):{Style.RESET_ALL}             {BackgroundColors.CYAN}R$ {tributos['cofins']:.2f}{Style.RESET_ALL}")
   print(f"- {BackgroundColors.RED}PIS (1.1694%):{Style.RESET_ALL}                {BackgroundColors.CYAN}R$ {tributos['pis']:.2f}{Style.RESET_ALL}\n")

   print(f"{BackgroundColors.BOLD}{BackgroundColors.GREEN}TOTAL ESTIMADO DA FATURA:       {BackgroundColors.CYAN}R$ {total:.2f}{Style.RESET_ALL}")
...
def main():
   ...
   print(f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Copel Bill Estimator{Style.RESET_ALL}", end="\n\n") # Output the Welcome message

   leitura_anterior, leitura_atual, consumo_kwh, itens_variaveis, tributos, total_fatura = calcular_fatura_copel(leitura_anterior=13726, leitura_atual=13706) # Call the function to calculate the Copel bill with example readings
   exibir_resultados(leitura_anterior, leitura_atual, consumo_kwh, itens_variaveis, tributos, total_fatura) # Display the results
