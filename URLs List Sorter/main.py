...
def sort_and_clean_urls():
    with open(INPUT_FILE, "r") as file:  # Open the file with the URLs
        ...
        lines.sort(
            key=lambda line: line[line.rfind("/") + 1 :]
...
def show_repeated_substrings(url_substring_counts):
    ...
    for substring, count in repeated_substrings.items():
        print(
            f"{BackgroundColors.GREEN}Substring: {BackgroundColors.CYAN}{substring}{BackgroundColors.GREEN}, Count: {BackgroundColors.CYAN}{count}{Style.RESET_ALL}"
        )
