...
DESIRED_REFACTORING_TYPES = [
    ...
    "Extract Superclass",
    "Move Method",
]  # The desired refactoring types
DEFAULT_REPOSITORY = "zookeeper"  # The default repository to be analyzed
FILES_TO_ANALYZE = {
    "org.apache.zookeeper.server.quorum.Leader": "lead",
    "org.apache.zookeeper.server.quorum.LeaderElection": "lookForLeader",
    "org.apache.zookeeper.server.quorum.Follower": "followLeader",
}  # The desired methods of each repository

# Constants:
CLASSES_TYPE = {"class", "interface", "enum", "innerclass", "anonymous"}  # The types of classes.
CLASSES_OR_METHODS = (
    "classes" if any(class_type in FILES_TO_ANALYZE.values() for class_type in CLASSES_TYPE) else "methods"
)  # The default class or method to be analyzed
...
RELATIVE_METRICS_EVOLUTION_DIRECTORY_PATH = (
    "../PyDriller/metrics_evolution"  # The relative path of the directory that contains the metrics evolution files
)
...
def generate_refactorings_concurrently(repository_name):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Generating the refactoring instances concurrently for the {BackgroundColors.CYAN}{list(FILES_TO_ANALYZE.items())} {CLASSES_OR_METHODS}{BackgroundColors.GREEN} in the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository...{Style.RESET_ALL}"
        )
    ...
    for classname, variable_attribute in tqdm(FILES_TO_ANALYZE.items(), desc="Processing Refactorings", unit="file"):
        thread = threading.Thread(
            target=generate_commit_refactorings_for_class_or_methods,
            args=(
                repository_name,
                classname,
                variable_attribute,
    ...
    for thread in tqdm(
        threads, desc="Joining Threads", unit="thread"
    ):  # For each thread, also show progress for joining
...
def generate_commit_refactorings_for_class_or_methods(repository_name, classname, variable_attribute):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Generating the refactoring instances for the {BackgroundColors.CYAN}{classname}{BackgroundColors.GREEN} {CLASSES_OR_METHODS} in the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository...{Style.RESET_ALL}"
        )
    ...
    create_directory(
        f"{FULL_JSON_FILES_DIRECTORY_PATH}{RELATIVE_METRICS_EVOLUTION_REFACTORINGS_DIRECTORY_PATH}/{repository_name}/{CLASSES_OR_METHODS}/{classname}/{variable_attribute}/",
        f"{RELATIVE_JSON_FILES_DIRECTORY_PATH}{RELATIVE_METRICS_EVOLUTION_REFACTORINGS_DIRECTORY_PATH}/{repository_name}/{CLASSES_OR_METHODS}/{classname}/{variable_attribute}",
    )
    create_directory(
        f"{FULL_JSON_FILES_DIRECTORY_PATH}{RELATIVE_METRICS_EVOLUTION_REFACTORINGS_DIRECTORY_PATH}/{repository_name}/{CLASSES_OR_METHODS}/{classname}/{variable_attribute}-filtered/",
        f"{RELATIVE_JSON_FILES_DIRECTORY_PATH}{RELATIVE_METRICS_EVOLUTION_REFACTORINGS_DIRECTORY_PATH}/{repository_name}/{CLASSES_OR_METHODS}/{classname}/{variable_attribute}-filtered",
    )
    ...
    for commit_hash in commits_hashes:  # For each commit, generate the JSON file
        ...
        thread = subprocess.Popen(
            [
                f"{FULL_REFACTORING_MINER_PATH}",
                "-c",
                f"{repository_directory_path}",
                f"{commit_hash}",
                "-json",
                f"{json_filepath}",
            ],
...
def filter_json_file(classname, json_filepath, json_filtered_filepath):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Filtering the JSON file {BackgroundColors.CYAN}{json_filepath}{BackgroundColors.GREEN} according to the desired refactoring types...{Style.RESET_ALL}"
        )
    ...
    for commit in json_data["commits"]:  # For each commit
        for refactoring in commit["refactorings"]:  # For each refactoring instance
            if (
                refactoring["type"] in DESIRED_REFACTORING_TYPES
            ):  # If the refactoring type is in the desired refactoring types
                for rightSideLocation, leftSideLocation in zip(
                    refactoring["rightSideLocations"], refactoring["leftSideLocations"]
                ):  # For each rightSideLocation and leftSideLocation
                    ...
                    if (
                        file_classname in rightSideLocation["filePath"]
                        or file_classname in leftSideLocation["filePath"]
                    ):  # If the file_classname is in the rightSideLocation or leftSideLocation
                        filtered_json_data.append(
                            refactoring
                        )  # Append the refactoring instance to the filtered JSON data
