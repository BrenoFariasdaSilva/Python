...
# Relative paths:
RELATIVE_REFACTORING_MINER_PATH = (
    "/RefactoringMiner-2.4.0/bin/RefactoringMiner"  # The relative path to the RefactoringMiner Tool
)
RELATIVE_JSON_FILES_DIRECTORY_PATH = (
    "/json_files"  # The relative path of the directory that contains the generated JSON files
)
RELATIVE_REPOSITORIES_REFACTORINGS_DIRECTORY_PATH = (
    "/repositories_refactorings"  # The relative path of the directory that contains the generated JSON files
)
RELATIVE_REPOSITORIES_DIRECTORY_PATH = (
    "/repositories"  # The relative path of the directory that contains the repositories
)
...
FULL_JSON_FILES_DIRECTORY_PATH = (
    START_PATH + RELATIVE_JSON_FILES_DIRECTORY_PATH
)  # The Full path of the directory that contains the generated JSON files
FULL_REPOSITORIES_DIRECTORY_PATH = (
    START_PATH + RELATIVE_REPOSITORIES_DIRECTORY_PATH
)  # The Full path of the directory that contains the repositories
...
def path_contains_whitespaces():
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Verifying if the {BackgroundColors.CYAN}PATH{BackgroundColors.GREEN} constant contains whitespaces...{Style.RESET_ALL}"
        )
...
def verify_refactorings():
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Verifying if the {BackgroundColors.CYAN}Refactorings{BackgroundColors.GREEN} for the {BackgroundColors.CYAN}DEFAULT_REFACTORINGS{BackgroundColors.GREEN} were already generated...{Style.RESET_ALL}"
        )
    ...
    for repository_name, repository_url in DEFAULT_REPOSITORIES.items():
        ...
        if not os.path.isfile(json_repository_filepath):
            ...
            print(
                f"{BackgroundColors.GREEN}The {BackgroundColors.CYAN}Refactorings{BackgroundColors.GREEN} for the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository were already generated.{Style.RESET_ALL}"
            )
...
def create_directory(full_directory_name, relative_directory_name):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Creating the {BackgroundColors.CYAN}{relative_directory_name}{BackgroundColors.GREEN} directory...{Style.RESET_ALL}"
        )
...
def process_repositories_concurrently(repositories):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Processing the repositories {BackgroundColors.CYAN}{list(repositories.keys())}{BackgroundColors.GREEN} concurrently...{Style.RESET_ALL}"
        )

    # Using ThreadPoolExecutor to manage threads and a tqdm progress bar
    with ThreadPoolExecutor(max_workers=len(repositories)) as executor, tqdm(
        total=len(repositories), desc="Processing Repositories"
    ) as progress:
        ...
        for repository_name, repository_url in repositories.items():
            ...
            output_time(
                estimated_time_string, commits_number / 2
            )  # Output the estimated time for running all of the iterations for the repository
...
def process_repository(repository_name, repository_url):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Processing the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository...{Style.RESET_ALL}"
        )
...
def clone_repository(repository_name, repository_url):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Cloning the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository...{Style.RESET_ALL}"
        )

    repository_directory_path = (
        f"{FULL_REPOSITORIES_DIRECTORY_PATH}/{repository_name}"  # The path to the repository directory
    )
    ...
    if os.path.isdir(repository_directory_path) and os.listdir(repository_directory_path):
        ...
        thread = subprocess.Popen(
            ["git", "clone", repository_url, repository_directory_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def update_repository(repository_name):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Updating the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository...{Style.RESET_ALL}"
        )
...
def generate_commit_refactorings(repository_name):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Generating the {BackgroundColors.CYAN}refactoring instances{BackgroundColors.GREEN} for the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository...{Style.RESET_ALL}"
        )
    ...
    thread = subprocess.Popen(
        [FULL_REFACTORING_MINER_PATH, "-a", repository_directory_path, "-json", json_output_filepath],
...
def output_time(output_string, time):
    ...
    print(
        f"{BackgroundColors.GREEN}{output_string}{BackgroundColors.CYAN}{rounded_time} {time_unit}{BackgroundColors.GREEN}.{Style.RESET_ALL}"
    )
...
def play_sound():
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Playing a {BackgroundColors.CYAN}sound{BackgroundColors.GREEN} when the program finishes...{Style.RESET_ALL}"
        )
...
def main():
    ...
    if path_contains_whitespaces():
        print(
            f"{BackgroundColors.RED}The {BackgroundColors.CYAN}{START_PATH}{BackgroundColors.RED} constant contains whitespaces. Please remove them!{Style.RESET_ALL}"
        )
    ...
    print(
        f"{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}RefactoringMiner{BackgroundColors.GREEN} script! This script is part of the {BackgroundColors.CYAN}Worked Example Miner (WEM){BackgroundColors.GREEN} project.{Style.RESET_ALL}"
    )
    print(
        f"{BackgroundColors.GREEN}This script will {BackgroundColors.CYAN}generate the refactoring file{BackgroundColors.GREEN} using {BackgroundColors.CYAN}RefactoringMiner{BackgroundColors.GREEN} for the {BackgroundColors.CYAN}{list(repositories.keys())}{BackgroundColors.GREEN} repositories.{Style.RESET_ALL}",
        end="\n\n",
    )

    # Create the json directory
    create_directory(
        f"{FULL_JSON_FILES_DIRECTORY_PATH}{RELATIVE_REPOSITORIES_REFACTORINGS_DIRECTORY_PATH}",
        f"{RELATIVE_JSON_FILES_DIRECTORY_PATH}{RELATIVE_REPOSITORIES_REFACTORINGS_DIRECTORY_PATH}",
    )
