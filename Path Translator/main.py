...
def is_english(text):
   ...
   verbose_output(f"{BackgroundColors.GREEN}Checking if the string is in English: {BackgroundColors.CYAN}{text}{Style.RESET_ALL}") # Output the verbose message
   ...
   try: # Try to detect the language of the text
      ...
      return text.lower() == translated.lower() # Check if the original text is equal to the translated text (case insensitive)
   except Exception as e: # If an error occurs during language detection
      print(f"{BackgroundColors.RED}⚠️ Failed to detect language for: {text}: {e}{Style.RESET_ALL}") # Output the error message
...
def safe_translate(text):
   ...
   verbose_output(f"{BackgroundColors.GREEN}Translating text: {BackgroundColors.CYAN}{text}{Style.RESET_ALL}") # Output the verbose message

   try: # Try to translate the text
      translated = GoogleTranslator(source="auto", target="en").translate(text) # Translate the text to English
      verbose_output(f'{BackgroundColors.GREEN}Translated "{text}" → "{translated}"{Style.RESET_ALL}') # Output the translated text
      return translated # Return the translated text
   except Exception as e: # If an error occurs during translation
      print(f'{BackgroundColors.RED}❌ Failed to translate text "{BackgroundColors.CYAN}{text}{BackgroundColors.RED}": {e}{Style.RESET_ALL}') # Output the error message
...
def rename_path_if_needed(path):
   ...
   if not is_english(base): # If the base name is not in English
      ...
      try: # Try to rename the file or directory
         ...
         print(f'{BackgroundColors.GREEN}📁 Renamed: "{BackgroundColors.CYAN}{base}{BackgroundColors.GREEN}" → "{BackgroundColors.CYAN}{translated_name}{BackgroundColors.GREEN}"{Style.RESET_ALL}') # Output the renamed path
         return new_path # Return the new path
      except Exception as e: # If an error occurs during renaming
         print(f'{BackgroundColors.RED}❌ Failed to rename "{BackgroundColors.CYAN}{path}{BackgroundColors.RED}": {e}{Style.RESET_ALL}') # Output the error message
...
def walk_and_translate(root="."):
   ...
   verbose_output(f"{BackgroundColors.GREEN}Walking through the directory structure starting from: {BackgroundColors.CYAN}{root}{Style.RESET_ALL}") # Output the verbose message
...
def main():
   ...
   print(f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Translating and renaming paths in {BackgroundColors.CYAN}{SEARCH_DIRS}{BackgroundColors.GREEN} directory(s) to English...{Style.RESET_ALL}") # Output the welcome message
