...
def verify_filepath_exists(filepath):
   ...
   verbose_output(f"{BackgroundColors.GREEN}Verifying if the file or folder exists at the path: {BackgroundColors.CYAN}{filepath}{Style.RESET_ALL}")
   ...
   for entry in INPUT_DIR.iterdir():
      ...
      if parsed:
         ...
         try:
            ...
            print(f"{BackgroundColors.RED}Error fetching year for {series_name} S{season_str}: {e}{Style.RESET_ALL}")
            year = "Unknown"
      ...
      # ------------------------------------------------------------
         ...
         print(f"{BackgroundColors.YELLOW}No season info found in '{entry.name}', scanning subdirectories...{Style.RESET_ALL}")
         for subentry in entry.iterdir():
            if subentry.is_dir():
               parsed_sub = parse_dir_name(subentry.name)
               if not parsed_sub:
                  print(f"{BackgroundColors.YELLOW}Skipping (no match in subdir): {subentry.name}{Style.RESET_ALL}")
                  continue
...
def main():
   ...
   print(f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Season Directory Renamer{BackgroundColors.GREEN} program!{Style.RESET_ALL}", end="\n\n")
   rename_dirs()
