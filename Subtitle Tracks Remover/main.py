...
def remove_subtitles(full_path):
   ...
   cmd = [
      ...
      temp_output, "-y" # Output file, overwrite if exists
   ]
...
def process_videos_in_directory():
   ...
   for full_path in tqdm(video_files, desc=f"Processing Videos", unit="file"): # Loop through all video files with a progress bar
      ...
      if not result: # If removing subtitles failed
         print(f"{BackgroundColors.RED}Failed to remove subtitles from: {BackgroundColors.CYAN}{full_path}{Style.RESET_ALL}")
...
def main():
   ...
   print(f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Subtitle Tracks Remover{BackgroundColors.GREEN} Program!{Style.RESET_ALL}") # Output the welcome message
