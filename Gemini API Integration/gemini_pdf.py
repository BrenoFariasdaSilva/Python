...
# File Path Constants:
CSV_INPUT_FILE = (
    "../PyDriller/metrics_statistics/zookeeper/substantial_CBO_classes_changes.csv"  # The path to the input JSON file
)
...
def load_csv_file(file_path):
    ...
    if not os.path.exists(file_path):
        print(
            f"{BackgroundColors.RED}The {BackgroundColors.CYAN}CSV file{BackgroundColors.RED} not found at {BackgroundColors.CYAN}{file_path}{Style.RESET_ALL}"
        )
        sys.exit(1)  # Exit the program
    ...
    if missing_columns:  # If there are missing columns
        print(
            f"{BackgroundColors.RED}The CSV file must contain {BackgroundColors.CYAN}{', '.join(missing_columns)}{BackgroundColors.RED} columns.{Style.RESET_ALL}"
        )
        sys.exit(1)  # Exit the program
...
def read_pdf_file(file_path=PDF_FILE):
    ...
    if not os.path.exists(file_path):
        print(
            f"{BackgroundColors.RED}PDF file {BackgroundColors.CYAN}{file_path}{BackgroundColors.RED} not found.{Style.RESET_ALL}"
        )
        sys.exit(1)  # Exit the program
...
def print_output(output):
    ...
    print(
        f"{BackgroundColors.BOLD}{BackgroundColors.CYAN}Output:{BackgroundColors.GREEN}\n{output}{Style.RESET_ALL}",
        end="\n",
    )  # Output the output
...
def handle_output(run, output):
    ...
    verbose_output(
        true_string=f"{BackgroundColors.GREEN}Handling the output of Run {BackgroundColors.CYAN}{run + 1}/{RUNS}...{Style.RESET_ALL}"
    )

    if RUNS > 1:  # If the number of runs is greater than 1
        print(
            f"{BackgroundColors.GREEN}Thread of Run {BackgroundColors.CYAN}{run + 1}/{RUNS}{BackgroundColors.GREEN} Finished.{Style.RESET_ALL}"
        )
...
def process_future_output(future, run, outputs):
    ...
    verbose_output(
        true_string=f"{BackgroundColors.GREEN}Thread of Run {BackgroundColors.CYAN}{run + 1}/{RUNS}{BackgroundColors.GREEN} Finished.{Style.RESET_ALL}"
    )
...
def perform_run(model, context_message, run_number, retries=3, backoff_factor=0.5):
    ...
    while attempt <= retries:  # While the attempt is less than or equal to the number of retries
        try:
            ...
            verbose_output(
                true_string=f"{BackgroundColors.YELLOW}Run {run_number} generated an exception: {exc}. Retrying in {wait_time:.2f} seconds...{Style.RESET_ALL}"
            )
            time.sleep(wait_time)  # Sleep the program

    raise Exception(
        f"{BackgroundColors.RED}Failed to perform run after {BackgroundColors.CYAN}{retries}{BackgroundColors.GREEN} retries.{Style.RESET_ALL}"
    )  # Raise an exception
...
def limited_thread_function(model, semaphore, message, run):
    ...
    verbose_output(
        true_string=f"{BackgroundColors.GREEN}Thread of Run {BackgroundColors.CYAN}{run + 1}/{RUNS}{BackgroundColors.GREEN} Started.{Style.RESET_ALL}"
    )
...
def perform_runs_with_threading(model, context_message):
    ...
    with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
        future_to_run = {
            executor.submit(limited_thread_function, model, semaphore, context_message, run): run for run in range(RUNS)
        }  # Create a future to run dictionary
...
def calculate_similarity_and_confidence(outputs, confidence=0.95):
    ...
    verbose_output(
        true_string=f"{BackgroundColors.GREEN}Calculating the similarity between the outputs...{Style.RESET_ALL}"
    )
    ...
    ci = stats.t.interval(
        confidence, len(similarities) - 1, loc=avg_similarity, scale=sem
...
def report_run_statistics(outputs):
    ...
    avg_similarity, confidence_interval = calculate_similarity_and_confidence(
        outputs, 0.95
    )  # Calculate the average similarity and confidence interval
    ...
    print(
        f"{BackgroundColors.BOLD}{BackgroundColors.GREEN}95% Confidence Interval for the Gemini's Output Similarity: {BackgroundColors.CYAN}({confidence_interval[0]:.2f}, {confidence_interval[1]:.2f}){Style.RESET_ALL}",
        end="\n\n",
    )
