...
def get_default_audio_index(file_path):
   ...
   cmd = [ # ffprobe command to get audio stream indices and default flags
      ...
      "-show_entries", "stream=index:stream_tags=default", # Show stream index and default tag
      "-of", "default=noprint_wrappers=1:nokey=1", # Output format
      file_path # Input file
   ]

   result = subprocess.run(cmd, capture_output=True, text=True).stdout.strip().split("\n") # Run ffprobe command and capture output
   ...
   return default_index if default_index is not None else result[0] if result else "0" # Return default index or first audio index or "0" if no audio streams
...
def get_audio_stream_indices(file_path):
   ...
   verbose_output(f"{BackgroundColors.GREEN}Retrieving audio stream indices for: {BackgroundColors.CYAN}{file_path}{Style.RESET_ALL}")

   try: # Prepare and execute ffprobe command to list audio stream indices
      probe_cmd = [
         "ffprobe", "-v", "error", # Suppress non-error output
         "-select_streams", "a", # Focus only on audio streams
         "-show_entries", "stream=index", # Display only the stream index
         "-of", "default=noprint_wrappers=1:nokey=1", # Format output with plain values
         file_path # File to analyze
      ]
      result = subprocess.run(probe_cmd, capture_output=True, text=True, check=True).stdout.strip() # Run and capture output
...
def main():
   ...
   print(f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Audio Track Cleaner{BackgroundColors.GREEN} program!{Style.RESET_ALL}", end="\n\n") # Output the welcome message
