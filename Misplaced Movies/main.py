...
def misplaced_folder(path_input):
    ...
    folder_list = os.listdir(
        rf"{path_input}"
    )  # List of folders in the path_input and rf is used to escape the backslashes
    ...
    for folder_name in folder_list:
        ...
        for file_name in file_list:
            if folder_name not in file_name:  # If the folder name is not in the file name
                print(
                    f"{BackgroundColors.CYAN}{file_name}{BackgroundColors.GREEN} is misplaced in {BackgroundColors.CYAN}{path_input}\{folder_name}{Style.RESET_ALL}"
                )
...
def move_files(path_input):
    ...
    number_of_files = [
        ...
        0,
        0,
    ]  # Related to the movies type, in that case, Dual, Dublado, Nacional, Legendado and English
...
def show_result(number_of_files):
    print(
        f"{BackgroundColors.GREEN}Total of files moved: {BackgroundColors.CYAN}{sum(number_of_files)}{Style.RESET_ALL}"
    )
    for i in range(len(movies_type)):
        print(
            f"{BackgroundColors.GREEN}Total of {BackgroundColors.CYAN}{movies_type[i]}{BackgroundColors.GREEN} files moved: {BackgroundColors.CYAN}{number_of_files[i]}{Style.RESET_ALL}"
        )
...
def main():
    path_input = input(
        f"{BackgroundColors.GREEN}Enter the path of the folder: {Style.RESET_ALL}"
