...
def main():
    print(
        f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Directories Files Set{BackgroundColors.GREEN} program!{Style.RESET_ALL}"
    )  # Output the welcome message

    first_dir = f"."
    second_dir = f"."

    common_files, files_only_in_dir1, files_only_in_dir2 = get_common_files(
        first_dir, second_dir
    )  # Get the common files, files only in the first directory, and files only in the second directory

    print(
        f"{BackgroundColors.GREEN}Common Files: {BackgroundColors.CYAN}{len(common_files)}{Style.RESET_ALL}", end="\n"
    )
    print(
        f"{BackgroundColors.CYAN}"
        + "\n".join([f"{i+1:02d} - {file}" for i, file in enumerate(format_files(common_files))])
        + f"{Style.RESET_ALL}\n"
    )

    print(
        f"{BackgroundColors.GREEN}Files Only in the First Directory: {BackgroundColors.CYAN}{len(files_only_in_dir1)}{Style.RESET_ALL}",
        end="\n",
    )
    print(
        f"{BackgroundColors.CYAN}"
        + "\n".join([f"{i+1:02d} - {file}" for i, file in enumerate(format_files(files_only_in_dir1))])
        + f"{Style.RESET_ALL}\n"
    )

    print(
        f"{BackgroundColors.GREEN}Files Only in the Second Directory: {BackgroundColors.CYAN}{len(files_only_in_dir2)}{Style.RESET_ALL}",
        end="\n",
    )
    print(
        f"{BackgroundColors.CYAN}"
        + "\n".join([f"{i+1:02d} - {file}" for i, file in enumerate(format_files(files_only_in_dir2))])
        + f"{Style.RESET_ALL}\n"
    )
