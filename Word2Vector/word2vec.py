    ...
    for idx, issue in enumerate(issue_titles):  # For each issue title
        for similar_issue in similar_titles:  # For each similar issue title
            if similar_issue.__len__() == 0:  # If the list is empty
                continue  # Continue to the next iteration
            print(
                f'{BackgroundColors.GREEN}Issue Nº{BackgroundColors.CYAN}{idx}{BackgroundColors.GREEN}: "{BackgroundColors.CYAN}{issue}{BackgroundColors.GREEN}" Similar to "{BackgroundColors.CYAN}{similar_issue[0][1]}{BackgroundColors.GREEN}" - similarity = {BackgroundColors.CYAN}{similar_issue[0][0]}{Style.RESET_ALL}'
            )
        print(f"")
...
def main():
    ...
    print(
        f"{BackgroundColors.GREEN}Number of Issues: {BackgroundColors.CYAN}{getIssuesCounter(issues_filepath)}{Style.RESET_ALL}"
    )
