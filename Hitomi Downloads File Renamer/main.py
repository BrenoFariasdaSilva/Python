...
def clean_filename(filename):
   ...
   verbose_output(f"{BackgroundColors.GREEN}Cleaning the filename: {BackgroundColors.CYAN}{filename}{Style.RESET_ALL}") # Output the verbose message
...
def rename_files_in_dir_recursively(base_dir):
   ...
   verbose_output(f"{BackgroundColors.GREEN}Renaming files in the directory: {BackgroundColors.CYAN}{base_dir}{Style.RESET_ALL}") # Output the verbose message
      ...
      for file in files: # For each file in the directory
         ...
         if file != cleaned_filename: # Rename only if the name was changed.
            ...
            try: # Try to rename the file
               ...
               print(f"{BackgroundColors.GREEN}Renamed file {BackgroundColors.CYAN}{original_path}{BackgroundColors.GREEN} to {BackgroundColors.CYAN}{new_path}{Style.RESET_ALL}") # Output the success message
            except Exception as e:
               print(f"{BackgroundColors.RED}Error renaming file {BackgroundColors.CYAN}{original_path}{BackgroundColors.RED} to {BackgroundColors.CYAN}{new_path}{BackgroundColors.RED}: {e}{Style.RESET_ALL}") # Output the error message
...
def main():
   ...
   print(f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Hitomi Downloader File Renamer{BackgroundColors.GREEN}!{Style.RESET_ALL}") # Output the welcome message

   root_directory = os.getcwd() # Get the current working directory
   rename_files_in_dir_recursively(root_directory) # Rename files in the current directory and its subdirectories recursively
