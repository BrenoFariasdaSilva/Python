...
def detect_missing_fields(bibtex_file):
   ...
   if not verify_filepath_exists(bibtex_file): # Check if the provided file exists
      print(f"{BackgroundColors.RED}Error: File not found at path {BackgroundColors.CYAN}{bibtex_file}{Style.RESET_ALL}")
      return
   ...
   if missing_field_entries: # If there are entries with missing fields
      ...
      for entry, fields in missing_field_entries.items():
         print(f" - {BackgroundColors.CYAN}{entry}{Style.RESET_ALL}: missing {BackgroundColors.RED}{", ".join(fields)}{Style.RESET_ALL}")
   else:
      print(f"{BackgroundColors.GREEN}All entries contain the required fields: {BackgroundColors.CYAN}{", ".join(FIELDS)}{Style.RESET_ALL}")
...
def main():
   ...
   print(f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}BibTex Find Missing Fields{BackgroundColors.GREEN}!{Style.RESET_ALL}", end="\n\n") # Output the welcome message
