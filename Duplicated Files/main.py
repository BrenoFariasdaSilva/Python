...
def find_duplicate_files(directory):
    ...
    for foldername, subfolders, filenames in tqdm(
        os.walk(directory), desc="Searching For Duplicated Files", unit="file"
    ):
...
def main():
    print(
        f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Welcome to {BackgroundColors.CYAN}Duplicated Files{BackgroundColors.GREEN}!{Style.RESET_ALL}"
    ...
    if duplicate_files:
        ...
        for filename, paths in tqdm(
            duplicate_files.items(), desc="Processing Duplicate files", unit="file"
        ):  # Loop through the duplicate files
    ...
    if DELETE_DUPLICATES:  # Delete the duplicate files
        for filename, paths in tqdm(duplicate_files.items(), desc="Deleting the Files Duplicates", unit="file"):
            ...
            print(
                f"{BackgroundColors.YELLOW}Deleted {BackgroundColors.CYAN}{len(paths)-1}{BackgroundColors.GREEN} copies of {BackgroundColors.CYAN}{filename}{BackgroundColors.GREEN}.{Style.RESET_ALL}"
            )
    else:
