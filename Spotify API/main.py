...
def create_directory(directory):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Creating the directory: {BackgroundColors.CYAN}{directory}{Style.RESET_ALL}"
    )  # Output the verbose message
...
def normalize(text):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Normalizing the text: {BackgroundColors.CYAN}{text}{Style.RESET_ALL}"
    )  # Output the verbose message
...
def load_json(filename):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Loading the JSON file: {BackgroundColors.CYAN}{filename}{Style.RESET_ALL}"
    )  # Output the verbose message
    ...
    try:  # Try to load the JSON file
        ...
        return {
            normalize(artist): {normalize(song) for song in songs} for artist, songs in data.items()
...
def songs_in_first_not_in_second(first, second):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Finding songs in the first dictionary that are not in the second dictionary.{Style.RESET_ALL}"
    )  # Output the verbose message

    return {
        f"{artist} - {song}"
        for artist, songs in first.items()
        for song in songs
        if artist not in second or song not in second[artist]
    }  # Return the set of songs in the first dictionary that are not in the second dictionary
...
def write_to_file(filename, data):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Writing data to the file: {BackgroundColors.CYAN}{filename}{Style.RESET_ALL}"
    )  # Output the verbose message
...
def compare_songs(spotify_file=SPOTIFY_LIKED_SONGS, local_file=LOCAL_SONGS):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Comparing Spotify liked songs with local songs.{Style.RESET_ALL}"
    )  # Output the verbose message
    ...
    local_not_in_spotify = songs_in_first_not_in_second(
        local_songs, spotify_songs
    )  # Find songs in local that are not in Spotify
    spotify_not_in_local = songs_in_first_not_in_second(
        spotify_songs, local_songs
    )  # Find songs in Spotify that are not in local

    write_to_file(
        LOCAL_SONGS_NOT_IN_SPOTIFY, "\n".join(local_not_in_spotify)
    )  # Write songs in local that are not in Spotify to a file
    write_to_file(
        SPOTIFY_SONGS_NOT_IN_LOCAL, "\n".join(spotify_not_in_local)
    )  # Write songs in Spotify that are not in local to a file
...
def main():
    ...
    print(
        f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Spotify API{BackgroundColors.GREEN} program!{Style.RESET_ALL}"
