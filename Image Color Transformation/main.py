...
def process_image(img_path: Path):
   ...
   print(f"{BackgroundColors.GREEN} Saved enhanced image: {BackgroundColors.CYAN}{output_path}{Style.RESET_ALL}\n") # Output the saved message
...
def main():
   ...
   print(f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Image Color Enhancer{BackgroundColors.GREEN}!{Style.RESET_ALL}", end="\n\n") # Output the Welcome message
   ...
   for img_path in INPUT_PATH.rglob("*"): # Recursively search for all files in the root folder
      if img_path.is_file() and img_path.suffix.lower() in IMAGE_EXTS: # If the file is an image
         if img_path.stem.lower() == TARGET_FILENAME.lower(): # If the file name is the same as the TARGET_FILENAME (case-insensitive)
