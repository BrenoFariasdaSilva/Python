...
}  # The dicitonary of the repositories to be analyzed (repository name: repository URL)
...
RELATIVE_CK_METRICS_DIRECTORY_PATH = (
    "/ck_metrics"  # The relative path of the directory that contains the CK generated files
)
...
RELATIVE_REFACTORINGS_DIRECTORY_PATH = (
    "/refactorings"  # The relative path of the directory that contains the refactorings
)
...
FULL_CK_METRICS_DIRECTORY_PATH = (
    START_PATH + RELATIVE_CK_METRICS_DIRECTORY_PATH
)  # The full path of the directory that contains the CK generated files
...
FULL_REFACTORINGS_DIRECTORY_PATH = (
    START_PATH + RELATIVE_REFACTORINGS_DIRECTORY_PATH
)  # The full path of the directory that contains the refactorings
...
def verify_ck_metrics_folder(repository_name):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Verifying if the metrics for {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} were already calculated...{Style.RESET_ALL}"
        )

    data_path = os.path.join(
        START_PATH, RELATIVE_CK_METRICS_DIRECTORY_PATH[1:]
    )  # Join the PATH with the relative path of the ck metrics directory
    ...
    for commit_hash in commit_hashes:  # Loop through the commit hashes
        ...
        if os.path.exists(folder_path):  # Verify if the folder exists
            for ck_metric_file in CK_METRICS_FILES:  # Verify if all the ck metrics files exist inside the folder
                ck_metric_file_path = os.path.join(
                    folder_path, ck_metric_file
                )  # Join the folder path with the ck metric file
...
def clone_repository(repository_name, repository_url):
    ...
    if os.path.isdir(repository_directory_path) and os.listdir(repository_directory_path):
        ...
        print(
            f"{BackgroundColors.GREEN}Successfully cloned the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository.{Style.RESET_ALL}"
        )
...
def get_last_execution_progress(repository_name, saved_progress_file, number_of_commits):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Getting the last execution progress of the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository...{Style.RESET_ALL}"
        )
    ...
    if os.path.exists(saved_progress_file):
        ...
        with open(saved_progress_file, "r") as progress_file:
            ...
            if len(lines) > 3:
                ...
                output_time(
                    execution_time, round(((number_of_commits / 1000) * (number_of_commits - last_commit_number)), 2)
                )  # Output the estimated time for running the remaining iterations for the repository

    else:  # If there is no saved progress file, create one and write the header
        with open(saved_progress_file, "w") as progress_file:  # Open the saved progress file
            progress_file.write(
                "Commit Number,Commit Hash,Commit Message,Commit Date\n"
...
def generate_diffs(repository_name, commit, commit_number):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Generating the diffs for the {BackgroundColors.CYAN}{commit_number}º{BackgroundColors.GREEN} commit of the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository...{Style.RESET_ALL}"
        )
    ...
    for modified_file in commit.modified_files:  # Loop through the modified files of the commit
        ...
        with open(
            f"{diff_file_directory}{modified_file.filename}{DIFF_FILE_EXTENSION}",
            "w",
            encoding="utf-8",
            errors="ignore",
        ) as diff_file:
...
def run_ck_metrics_generator(cmd):
    ...
    )  # Run the cmd command in a subprocess
...
def show_execution_time(first_iteration_duration, elapsed_time, number_of_commits, repository_name):
    ...
    output_time(
        estimated_time_string, round(first_iteration_duration * number_of_commits, 2)
    ...
    output_time(
        time_taken_string, round(elapsed_time, 2)
    )  # Output the time taken to generate CK metrics for the commits in the repository
...
def traverse_repository(repository_name, repository_url, number_of_commits):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Traversing the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository to run CK for every commit hash...{Style.RESET_ALL}"
        )
    ...
    with tqdm(
        total=number_of_commits - last_commit,
        unit=f"{BackgroundColors.GREEN}Traversing the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} commit tree{Style.RESET_ALL}",
        unit_scale=True,
    ) as pbar:
        for commit in Repository(repository_url).traverse_commits():  # Loop through the commits of the repository
            ...
            output_directory, relative_output_directory = generate_output_directory_paths(
                repository_name, commit.hash, commit_number
            )
...
def process_repository(repository_name, repository_url):
    ...
    if verify_ck_metrics_folder(repository_name):
        print(
            f"{BackgroundColors.GREEN}The metrics for {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} were already calculated!{Style.RESET_ALL}"
        )
        return
...
def process_repositories_in_parallel():
    ...
    for repository_name, repository_url in DEFAULT_REPOSITORIES.items():
        ...
        output_time(
            estimated_time_string, round(((commits_number / 1000) * commits_number), 2)
        )  # Output the estimated time for running all of the iterations for the repository
        ...
        )  # Create a thread to process the repository
...
def main():
    ...
    if path_contains_whitespaces():
        print(
            f"{BackgroundColors.RED}The {START_PATH} constant contains whitespaces. Please remove them!{Style.RESET_ALL}"
        )
        return
    ...
    if not os.path.exists(FULL_CK_JAR_PATH):
        print(
            f"{BackgroundColors.RED}The CK JAR file does not exist. Please download it and place it in {BackgroundColors.CYAN}{RELATIVE_CK_JAR_PATH[0:RELATIVE_CK_JAR_PATH.find('/', 1)]}/{BackgroundColors.RED}.{Style.RESET_ALL}"
        )
        return

    # Print the welcome message
    print(
        f"{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}CK Metrics Generator{BackgroundColors.GREEN}! This script is a key component of the {BackgroundColors.CYAN}Worked Example Miner (WEM) Project{BackgroundColors.GREEN}.{Style.RESET_ALL}"
    )
    print(
        f"{BackgroundColors.GREEN}This script will process the repositories: {BackgroundColors.CYAN}{list(DEFAULT_REPOSITORIES.keys())}{BackgroundColors.GREEN} in parallel using threads.{Style.RESET_ALL}"
    )
    print(
        f"{BackgroundColors.GREEN}The files that this script will generate are the {BackgroundColors.CYAN}ck metrics files, the commit hashes list file and the diffs of each commit{BackgroundColors.GREEN}, in which are used by the {BackgroundColors.CYAN}Metrics Changes{BackgroundColors.GREEN} Python script.{Style.RESET_ALL}",
        end="\n\n",
    )
