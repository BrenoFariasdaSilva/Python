...
from code_metrics import (
    ...
    FULL_REFACTORINGS_DIRECTORY_PATH,
...
from code_metrics import (
    create_directory,
    output_time,
    path_contains_whitespaces,
    play_sound,
    verify_ck_metrics_folder,
)  # Importing functions from the code_metrics.py file
...
DESIRED_REFACTORINGS = [
    ...
    "Extract Superclass",
    "Move Method",
]  # The desired refactorings to search for substantial changes
...
# Filenames:
CK_CSV_FILE = (
    CK_METRICS_FILES[0] if PROCESS_CLASSES else CK_METRICS_FILES[1]
)  # The name of the csv generated file from ck.
...
# Relative Paths:
RELATIVE_METRICS_DATA_DIRECTORY_PATH = (
    "/metrics_data"  # The relative path to the directory containing the metrics evolution
)
RELATIVE_METRICS_EVOLUTION_DIRECTORY_PATH = (
    "/metrics_evolution"  # The relative path to the directory containing the metrics evolution
)
RELATIVE_METRICS_STATISTICS_DIRECTORY_PATH = (
    "/metrics_statistics"  # The relative path to the directory containing the metrics statistics
)
RELATIVE_METRICS_PREDICTION_DIRECTORY_PATH = (
    "/metrics_predictions"  # The relative path to the directory containing the metrics prediction
)
...
def update_global_variables():
    ...
    PROCESS_CLASSES = (
        input(
            f"{BackgroundColors.GREEN}Do you want to process the {BackgroundColors.CYAN}class.csv{BackgroundColors.GREEN} file {BackgroundColors.RED}(True/False){BackgroundColors.GREEN}? {Style.RESET_ALL}"
        )
        .strip()
        .lower()
        == "true"
    ...
    UNSORTED_CHANGED_METHODS_CSV_FILENAME = (
        f"{CK_CSV_FILE.replace('.csv', '')}_unsorted_changes.{CK_CSV_FILE.split('.')[1]}"
    )
    SORTED_CHANGED_METHODS_CSV_FILENAME = f"{CK_CSV_FILE.replace('.csv', '')}_changes.{CK_CSV_FILE.split('.')[1]}"
    SUBSTANTIAL_CHANGES_FILENAME = (
        f"substantial_{SUBSTANTIAL_CHANGE_METRIC}_{CLASSES_OR_METHODS}_changes{CSV_FILE_EXTENSION}"
    )
...
def process_repository(repository_name):
    ...
    if not verify_ck_metrics_folder(repository_name):
        print(
            f"{BackgroundColors.RED}The metrics for {BackgroundColors.CYAN}{repository_name}{BackgroundColors.RED} were not calculated. Please run the {BackgroundColors.CYAN}code_metrics.py{BackgroundColors.RED} file first{Style.RESET_ALL}"
        )
    ...
    old_csv_file_path = (
        f"{FULL_METRICS_STATISTICS_DIRECTORY_PATH}/{repository_name}/{UNSORTED_CHANGED_METHODS_CSV_FILENAME}"
    )
...
def get_directory_path(repository_name):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Getting the directory path for the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository...{Style.RESET_ALL}"
        )
...
def create_directories(repository_name):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Creating the desired directories for the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository...{Style.RESET_ALL}"
        )
    ...
    create_directory(
        f"{FULL_METRICS_DATA_DIRECTORY_PATH}/{repository_name}",
        f"{RELATIVE_METRICS_DATA_DIRECTORY_PATH}/{repository_name}",
    )
    ...
    create_directory(
        f"{FULL_METRICS_EVOLUTION_DIRECTORY_PATH}/{repository_name}/{CLASSES_OR_METHODS}",
        f"{RELATIVE_METRICS_EVOLUTION_DIRECTORY_PATH}/{repository_name}/{CLASSES_OR_METHODS}",
    ...
    create_directory(
        f"{FULL_METRICS_STATISTICS_DIRECTORY_PATH}/{repository_name}",
        f"{RELATIVE_METRICS_STATISTICS_DIRECTORY_PATH}/{repository_name}",
    )
    ...
    create_directory(
        f"{FULL_METRICS_PREDICTION_DIRECTORY_PATH}/{repository_name}/{CLASSES_OR_METHODS}",
        f"{RELATIVE_METRICS_PREDICTION_DIRECTORY_PATH}/{repository_name}/{CLASSES_OR_METHODS}",
    )
...
def valid_class_name(class_name):
    ...
    return (
        "." in class_name
    )  # If the class name contains a dot, then it is valid (it is a package name) and returns True
...
def get_class_package_name(file_name):
    ...
    package_name = file_name[
        file_name.find(start_substring) + len(start_substring) : file_name.rfind(".")
    ]  # Get the substring that comes after the: /src/ and before the last dot
...
def was_file_modified(commit_modified_files_dict, commit_hash, row):
    ...
    file_path = row["file"][
        row["file"].find(FULL_REPOSITORIES_DIRECTORY_PATH) + len(FULL_REPOSITORIES_DIRECTORY_PATH) + 1 :
    ]
    ...
    modified_files_paths = commit_modified_files_dict[
        commit_hash
    ]  # Get the modified files list for the specified commit hash
...
def process_csv_file(commit_modified_files_dict, file_path, metrics_track_record):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Processing the csv file containing the metrics of a method nor class...{Style.RESET_ALL}"
        )
    ...
    with open(file_path, "r") as csvfile:
        ...
        for row in reader:
            ...
            if (commit_hash not in commit_hashes or (ck_metrics not in metrics_changes)) and (
                was_file_modified(commit_modified_files_dict, commit_hash, row)
            ):
...
def generate_commit_modified_files_dict(repository_name):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Generating the commit dictionary for the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository...{Style.RESET_ALL}"
        )

    commit_modified_files_dict = (
        {}
    )  # A dictionary containing the commit hashes as keys and the modified files path list as values
    ...
    for commit in Repository(DEFAULT_REPOSITORIES[repository_name]).traverse_commits():
        ...
        for modified_file in commit.modified_files:  # For each modified file in the commit
            commit_modified_files_dict[commit.hash].append(
                modified_file.new_path
            )  # Append the modified file path to the commit diff d

    return (
        commit_modified_files_dict  # Return the commit dictionary containing the modified files paths for each commit
    )
...
def traverse_directory(repository_name, repository_ck_metrics_path):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Traversing the directory and processing all the csv files for the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository...{Style.RESET_ALL}"
        )

    metrics_track_record = (
        {}
    )  # Dictionary containing the track record of the metrics of each method nor class. The key is the identifier and the value is a dictionary containing the metrics, commit hashes and the number of times the metrics changed.
    ...
    with tqdm(
        total=len(os.listdir(repository_ck_metrics_path)),
        unit=f" {BackgroundColors.CYAN}{repository_ck_metrics_path.split('/')[-1]} files{Style.RESET_ALL}",
    ) as progress_bar:
            ...
            for dir in subdirs:  # For each subdirectory in the directories
                for file in os.listdir(os.path.join(root, dir)):  # For each file in the subdirectory
                    if file == CK_CSV_FILE:  # If the file is the desired csv file
                        ...
                        process_csv_file(
                            commit_modified_files_dict, file_path, metrics_track_record
...
def write_metrics_track_record_to_txt(repository_name, metrics_track_record):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Writing the metrics track record for the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository to a txt file...{Style.RESET_ALL}"
        )

    # Open the txt file and write the metrics_track_record to it
    with open(
        f"{FULL_METRICS_DATA_DIRECTORY_PATH}/{repository_name}/{CLASSES_OR_METHODS}_track_record.txt", "w"
    ) as file:
...
def get_clean_id(id):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Getting the clean id for {BackgroundColors.CYAN}{id}{BackgroundColors.GREEN}...{Style.RESET_ALL}"
        )
...
def verify_file(file_path):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Verifying the file for {BackgroundColors.CYAN}{file_path}{BackgroundColors.GREEN}...{Style.RESET_ALL}"
        )
...
def get_refactorings_info(repository_name, commit_number, commit_hash, class_name):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Getting the specific informations about the refactorings for {BackgroundColors.CYAN}{class_name}{BackgroundColors.GREEN} in the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository...{Style.RESET_ALL}"
        )
        ...
        for commit in data["commits"]:
            if commit["sha1"] == commit_hash:  # If the commit hash is equal to the specified commit hash
                for refactoring in commit["refactorings"]:  # Loop through the refactorings in the commit
                    for location in (
                        refactoring["leftSideLocations"] + refactoring["rightSideLocations"]
                    ):  # Loop through the locations in the refactoring
                        ...
                        if class_name.replace(".", "/") in location["filePath"]:
                            refactorings_info["types"].append(
                                refactoring["type"]
                            )  # Append the refactoring type to the refactorings list
                            if (
                                location["filePath"] not in refactorings_info["filePath"]
                            ):  # If the file path is not in the refactorings_info["filePath"] list
...
def generate_refactoring_file(repository_name, commit_hash, refactoring_file_path):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Generating the refactoring file for the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository...{Style.RESET_ALL}"
        )
    ...
    if (
        not verify_file(refactoring_file_path) or os.path.getsize(refactoring_file_path) == 0
    ):  # If the refactoring file does not exist
    ...
    if is_valid:  # If the refactoring file was properly generated
        ...
        print(
            f"{BackgroundColors.RED}The refactoring file for the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.RED} repository was not generated: {BackgroundColors.YELLOW}{message}{Style.RESET_ALL}"
        )
        return None  # Return None


def verify_substantial_metric_decrease(
    metrics_values, class_name, raw_variable_attribute, commit_hashes, metric_name, repository_name
):
    """
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Verifying if the class or method has had a substantial decrease in the {BackgroundColors.CYAN}{metric_name}{BackgroundColors.GREEN} metric...{Style.RESET_ALL}"
        )
    ...
    for i in range(1, len(metrics_values)):
        ...
        if current_percentual_variation > DESIRED_DECREASED and current_percentual_variation > biggest_change[2]:
            ...
            temp_commit_data = [
                commit_hashes[i - 1].split("-")[0],
                commit_hashes[i - 1].split("-")[1],
            ]  # The commit data [commit_number, commit_hash]
            refactorings_info = get_refactorings_info(
                repository_name, temp_commit_data[0], temp_commit_data[1], class_name
            )  # Get the refactorings information

            # Verify if we're not filtering by desired refactorings or if the current refactoring type is a desired refactoring.
            if not DESIRED_REFACTORINGS_ONLY or any(
                refactoring in DESIRED_REFACTORINGS for refactoring in refactorings_info["types"]
            ):
                # Update the biggest_change list and commit data only if the conditions above are met.
                biggest_change = [
                    metrics_values[i - 1],
                    metrics_values[i],
                    current_percentual_variation,
                    refactorings_info["types"],
                    refactorings_info["filePath"],
                ]
    ...
    if biggest_change[2] > DESIRED_DECREASED:
        with open(f"{csv_filename}", "a") as csvfile:  # Open the csv file
            ...
            writer.writerow(
                [
                    class_name,
                    raw_variable_attribute,
                    biggest_change[0],
                    biggest_change[1],
                    round(biggest_change[2] * 100, 2),
                    commit_data[0],
                    commit_data[1],
                    refactorings_info["types"],
                    refactorings_info["filePath"],
...
def linear_regression_graphics(
    metrics, class_name, variable_attribute, commit_hashes, raw_variable_attribute, repository_name
):
    """
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Performing linear regression on the given metrics and saving the plot to a PNG file for {BackgroundColors.CYAN}{class_name}{BackgroundColors.GREEN} in the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository...{Style.RESET_ALL}"
        )
    ...
    try:
        ...
        if VERBOSE:
            print(
                f"{BackgroundColors.RED}Error converting the {BackgroundColors.CYAN}metrics{BackgroundColors.GREEN} to {BackgroundColors.CYAN}NumPy array{BackgroundColors.GREEN} for {class_name} {variable_attribute}.{Style.RESET_ALL}"
            )
        return
    ...
    if metrics_array.ndim != 2 or metrics_array.shape[1] < len(METRICS_POSITION):
        if VERBOSE:
            print(
                f"{BackgroundColors.RED}Metrics structure for {class_name} {variable_attribute} is not as expected!{Style.RESET_ALL}"
            )
    ...
    for metric_name, metric_position in METRICS_POSITION.items():
        ...
        metric_values = metrics_array[
            :, metric_position
        ]  # Extract the metrics values from the metrics array in the specified position (column)
        ...
        if metric_name == SUBSTANTIAL_CHANGE_METRIC:
            verify_substantial_metric_decrease(
                metric_values, class_name, raw_variable_attribute, commit_hashes, metric_name, repository_name
            )
        ...
        plt.savefig(
            f"{FULL_METRICS_PREDICTION_DIRECTORY_PATH}/{repository_name}/{CLASSES_OR_METHODS}/{class_name}/{variable_attribute}/{metric_name}{PNG_FILE_EXTENSION}"
        )
...
def write_metrics_evolution_to_csv(repository_name, metrics_track_record):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Writing the metrics evolution for the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository to a csv file...{Style.RESET_ALL}"
        )

    # For every identifier in the metrics_track_record, store each metrics values tuple in a row of the csv file
    with tqdm(
        total=len(metrics_track_record),
        unit=f" {BackgroundColors.CYAN}Creating Linear Regression and Metrics Evolution{Style.RESET_ALL}",
    ) as progress_bar:
        for (
            identifier,
            record,
        ) in metrics_track_record.items():  # For each identifier and record in the metrics_track_record dictionary
            ...
            variable_attribute = get_clean_id(
                identifier.split(" ")[1]
            )  # Get the variable attribute which could be the type of the class or the method name
            ...
            create_directory(
                full_metrics_evolution_directory_path,
                f"{RELATIVE_METRICS_EVOLUTION_DIRECTORY_PATH}/{repository_name}/{CLASSES_OR_METHODS}/{class_name}",
            )  # Create the directory where the csv file will be stored
            ...
            with open(metrics_filename, "w") as csvfile:
                ...
                for i in range(metrics_len):  # For each metric in the metrics list
                    ...
                    writer.writerow(
                        [unique_identifier, record["commit_hashes"][i], metrics[i][0], metrics[i][1], metrics[i][2]]
                    )

            # Perform linear regression and generate graphics for the metrics
            linear_regression_graphics(
                metrics,
                class_name,
                variable_attribute,
                record["commit_hashes"],
                identifier.split(" ")[1],
                repository_name,
            )
            progress_bar.update(1)  # Update the progress bar
...
def write_method_metrics_statistics(csv_writer, id, key, metrics, metrics_values, first_commit_hash, last_commit_hash):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Calculating the minimum, maximum, average, and third quartile of each metric and writing it to a csv file for {BackgroundColors.CYAN}{id}{BackgroundColors.GREEN}...{Style.RESET_ALL}"
        )
    ...
    cboAvg = round(
        float(sum(metrics_values[0])) / len(metrics_values[0]), 3
    )  # The average cbo value rounded to 3 decimal places
    cboQ3 = round(
        float(np.percentile(metrics_values[0], 75)), 3
    )  # The third quartile cbo value rounded to 3 decimal places
    ...
    wmcAvg = round(
        float(sum(metrics_values[1])) / len(metrics_values[1]), 3
    )  # The average wmc value rounded to 3 decimal places
    wmcQ3 = round(
        float(np.percentile(metrics_values[1], 75)), 3
    )  # The third quartile wmc value rounded to 3 decimal places
    ...
    rfcAvg = round(
        float(sum(metrics_values[2])) / len(metrics_values[2]), 3
    )  # The average rfc value rounded to 3 decimal places
    rfcQ3 = round(
        float(np.percentile(metrics_values[2], 75)), 3
    )  # The third quartile rfc value rounded to 3 decimal places
...
def generate_metrics_track_record_statistics(repository_name, metrics_track_record):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Processing the metrics in the {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN} repository to calculate the minimum, maximum, average, and third quartile of each metric and writing it to a csv file...{Style.RESET_ALL}"
        )

    # Open the csv file and process the metrics of each method
    unsorted_metrics_filename = (
        f"{FULL_METRICS_STATISTICS_DIRECTORY_PATH}/{repository_name}/{UNSORTED_CHANGED_METHODS_CSV_FILENAME}"
    )
    with open(unsorted_metrics_filename, "w") as csvfile:
        ...
        with tqdm(
            total=len(metrics_track_record),
            unit=f" {BackgroundColors.CYAN}Creating Metrics Statistics{Style.RESET_ALL}",
        ) as progress_bar:
            for identifier, metrics in metrics_track_record.items():
                ...
                write_method_metrics_statistics(
                    writer,
                    id,
                    key,
                    metrics,
                    metrics_values,
                    metrics_track_record[identifier]["commit_hashes"][0],
                    metrics_track_record[identifier]["commit_hashes"][-1],
                )
                progress_bar.update(1)  # Update the progress bar
...
def sort_csv_by_changes(repository_name):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Sorting the {BackgroundColors.CYAN}metrics statistics files{BackgroundColors.GREEN} by the {BackgroundColors.CYAN}number of changes{BackgroundColors.GREEN}.{Style.RESET_ALL}"
        )
    ...
    data.to_csv(
        f"{FULL_METRICS_STATISTICS_DIRECTORY_PATH}/{repository_name}/{SORTED_CHANGED_METHODS_CSV_FILENAME}", index=False
    )
...
def sort_csv_by_percentual_variation(repository_name):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Sorting the {BackgroundColors.CYAN}interesting changes files{BackgroundColors.GREEN} by the {BackgroundColors.CYAN}percentual variation of the metric{BackgroundColors.GREEN}.{Style.RESET_ALL}"
        )
    ...
    # Write the sorted csv file to a new csv file
    data.to_csv(
        f"{FULL_METRICS_STATISTICS_DIRECTORY_PATH}/{repository_name}/{SUBSTANTIAL_CHANGES_FILENAME}", index=False
    )
...
def main():
    ...
    if not verify_file(RELATIVE_REFACTORING_MINER_DIRECTORY_PATH):
        print(
            f"{BackgroundColors.RED}The {BackgroundColors.CYAN}RefactoringMiner{BackgroundColors.RED} tool was not found in the specified path: {BackgroundColors.GREEN}{RELATIVE_REFACTORING_MINER_DIRECTORY_PATH}{Style.RESET_ALL}"
        )
        return  # Exit the program

    # Print the welcome message
    print(
        f"{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Metrics Changes Generator{BackgroundColors.GREEN}! This script is part of the {BackgroundColors.CYAN}Worked Example Miner (WEM){BackgroundColors.GREEN} project.{Style.RESET_ALL}"
    )
    print(
        f"{BackgroundColors.GREEN}This script generates the {BackgroundColors.CYAN}classes or methods metrics evolution history, metrics statistics and linear regression{BackgroundColors.GREEN} for the {BackgroundColors.CYAN}{list(DEFAULT_REPOSITORY_NAMES)}{BackgroundColors.GREEN} repositories based on the {BackgroundColors.CYAN}ck metrics files, the commit hashes list file and the diffs of each commit{BackgroundColors.GREEN} generated by the {BackgroundColors.CYAN}./code_metrics.py{BackgroundColors.GREEN} code.{Style.RESET_ALL}",
        end="\n\n",
    )
