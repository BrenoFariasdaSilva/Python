...
def search_empty_folders(directory):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Searching for empty folders in {BackgroundColors.CYAN}{directory}{Style.RESET_ALL}"
        )
...
def main():
    ...
    print(
        f"{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Empty Folders Finder{BackgroundColors.GREEN}! This script is part of the {BackgroundColors.CYAN}Worked Example Miner (WEM){BackgroundColors.GREEN} project.{Style.RESET_ALL}"
    )
    print(
        f"{BackgroundColors.GREEN}This script searches for empty folders in the specified directories. Empty folders could be a sign of missing data or incomplete processing.{Style.RESET_ALL}",
        end="\n\n",
    )
    ...
    initial_directory = input(
        f"{BackgroundColors.GREEN}Enter the initial directory {BackgroundColors.CYAN}(default, relative/absolute paths): {Style.RESET_ALL}"
    )
    print(f"")
    ...
    for directory in initial_directory:
        ...
        if empty_folders:  # If empty folders are found
            print(
                f"{BackgroundColors.GREEN}Empty folders found in {BackgroundColors.CYAN}{directory}{BackgroundColors.GREEN}:{Style.RESET_ALL}"
            )
            ...
            print(
                f"{BackgroundColors.GREEN}Total empty folders found in {BackgroundColors.CYAN}{directory}{BackgroundColors.GREEN}: {BackgroundColors.CYAN}{len(empty_folders)}{Style.RESET_ALL}"
            )
            print(f"")
        else:  # If no empty folders are found
            print(
                f"{BackgroundColors.GREEN}No empty folders found in {BackgroundColors.CYAN}{directory}{BackgroundColors.GREEN}.{Style.RESET_ALL}"
            )
