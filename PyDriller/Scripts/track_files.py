...
def search_files(search_directory, search_string):
    ...
    if VERBOSE:  # If the VERBOSE constant is set to True
        print(
            f"{BackgroundColors.GREEN}Searching for {BackgroundColors.CYAN}{search_string} {BackgroundColors.GREEN}files in {BackgroundColors.CYAN}{search_directory}{Style.RESET_ALL}"
        )
...
def main():
    ...
    print(
        f"{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Track Files{BackgroundColors.GREEN} script! This script is part of the {BackgroundColors.CYAN}Worked Example Miner (WEM){BackgroundColors.GREEN} project.{Style.RESET_ALL}"
    )
    print(
        f"{BackgroundColors.GREEN}This script searches for {BackgroundColors.CYAN}specific files (README.md, for example){BackgroundColors.GREEN} in the specified repositories and writes the file paths to a text file stored in the {BackgroundColors.CYAN}metrics_data{BackgroundColors.GREEN} directory.{Style.RESET_ALL}",
        end="\n\n",
    )
    ...
    for repository_name in REPOSITORIES:
        print(
            f"{BackgroundColors.GREEN}Searching for {BackgroundColors.CYAN}changes files{BackgroundColors.GREEN} in {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN}...{Style.RESET_ALL}"
        )
        ...
        if not os.path.isdir(search_directory):
            print(
                f"{BackgroundColors.RED}Directory {BackgroundColors.CYAN}PyDriller/diffs/{repository_name}{BackgroundColors.RED} does not exist{Style.RESET_ALL}"
            )
            continue
        ...
        if not TARGET_FILENAMES[repository_name]:
            print(
                f"{BackgroundColors.RED}Target file name for {BackgroundColors.CYAN}{repository_name}{BackgroundColors.RED} is empty{Style.RESET_ALL}"
            )
            continue
        ...
        print(
            f"{BackgroundColors.GREEN}Number of {BackgroundColors.CYAN}{TARGET_FILENAMES[repository_name]} {BackgroundColors.GREEN}files found in {BackgroundColors.CYAN}{repository_name}{BackgroundColors.GREEN}: {BackgroundColors.CYAN}{found_files_count}{Style.RESET_ALL}"
        )
