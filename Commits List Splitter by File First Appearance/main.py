...
# File Structure:
FIELDNAMES = ["Commit Number", "Commit Hash", "Commit Date", "Commit Message", "Files Changed"] # List of field names for the CSV files
...
def normalize_file_list(raw):
   ...
   verbose_output(f"{BackgroundColors.GREEN}Normalizing file list from raw string: {BackgroundColors.CYAN}{raw}{Style.RESET_ALL}") # Output the verbose message
...
def file_matches_any(filename, patterns):
   ...
   verbose_output(f"{BackgroundColors.GREEN}Checking if file '{BackgroundColors.CYAN}{filename}{BackgroundColors.GREEN}' matches any of the patterns: {BackgroundColors.CYAN}{patterns}{Style.RESET_ALL}") # Output the verbose message
...
def should_track(filename):
   ...
   verbose_output(f"{BackgroundColors.GREEN}Determining if file '{BackgroundColors.CYAN}{filename}{BackgroundColors.GREEN}' should be tracked based on patterns.{Style.RESET_ALL}") # Output the verbose message
...
def sanitize_filename(name):
   ...
   verbose_output(f"{BackgroundColors.GREEN}Sanitizing filename: {BackgroundColors.CYAN}{name}{Style.RESET_ALL}") # Output the verbose message
...
def write_commits_to_csv(out_path, commits, fieldnames=FIELDNAMES):
   ...
   verbose_output(f"{BackgroundColors.GREEN}Writing {len(commits)} commits to CSV: {BackgroundColors.CYAN}{out_path}{Style.RESET_ALL}") # Log the action
...
def split_commits_by_first_file_appearance(csv_path):
   ...
   verbose_output(f"{BackgroundColors.GREEN}Splitting commits by first file appearance from CSV: {BackgroundColors.CYAN}{csv_path}{Style.RESET_ALL}") # Output the verbose message
...
def main():
   ...
   if not files: 
      print(f"{BackgroundColors.YELLOW}No files matched the tracking criteria. No CSV files were created.{Style.RESET_ALL}") # Output no files created message
      return # Exit the program
   ...
   for idx, f in enumerate(files, 1): # Iterate through files with their first-appearance index
      ...
      print(f"{BackgroundColors.CYAN}{idx}. {BackgroundColors.GREEN}{f} {BackgroundColors.CYAN}- {commit_number} - {commit_date} - {commit_hash} {BackgroundColors.GREEN}- {commit_message}{Style.RESET_ALL}")
