...
coords = {
    ...
    "btnHeroUnselect": {"xStart": 0.0875, "xEnd": 0.520833333, "xDelta": 0.0658333333, "y": 0.259259259},
    "btnHeroSelect": {
        "xStart": 0.104166667,
        "xEnd": 0.940416667,
        "xDelta": 0.0758333333,
    ...
    "btnAvoidAction": {"x": 0.52125, "y": 0.0388888889},
    "btnSwipeToPreviousPage": {"xStart": 0.0291666667, "xEnd": 0.492916667, "y": 0.62962963},
    "btnSwipeToNextPage": {"xStart": 0.916666667, "xEnd": 0.25, "y": 0.509259259},
    "btnNext": {"x": 0.887037037, "y": 0.92962963},
    "btnPVPFreePlay": {"x": 0.22125, "y": 0.511111111},
    "btnOpenPVPCrate": {"x": 0.52625, "y": 0.87962963},
}
...
while True:  # ((now.hour <= nextResetHour) and (now.minute <= nextResetMinute)):
    ...
    for unselectHeroXCoordinate in range(
        xStartUnselectHeroes, xEndUnselectHeroes, xDeltaUnselectHeroes
    ):  # Unselect Heroes
        device.shell(
            f"input touchscreen swipe {unselectHeroXCoordinate} {yValueUnselectHeroes} {unselectHeroXCoordinate} {yValueUnselectHeroes} 100"
        )  # Hero Unselect
    ...
    for pagesSwipeCounter in range(0, amountOfHeroesPages - 2):  # Swipe all back to left
        device.shell(
            f"input touchscreen swipe {xStartSwipeToStart} {yValueSwipeToStart} {xEndSwipeToStart} {yValueSwipeToStart} {swipeToStartDuration}"
        )
    time.sleep(1.0)
    ...
    for pagesSwipeCounter in range(0, amountOfHeroesPages):
        for columnCoordinates in range(xStartHeroSelect, xEndHeroSelect, xDeltaHeroSelect):  # Change column
            for lineCoordinates in range(yStartHeroSelect, yEndHeroSelect, yDeltaHeroSelect):  # Change line
                device.shell(
                    f"input touchscreen swipe {columnCoordinates} {lineCoordinates} {columnCoordinates} {lineCoordinates} 100"
                )  # Hero Select
                ...
                device.shell(
                    f"input touchscreen swipe {xAvoidAction} {yAvoidAction} {xAvoidAction} {yAvoidAction} 100"
                )  # Avoid "Revive Hero Lives"

        # if (pagesSwipeCounter == amountOfHeroesPages - 1):
        ...
        #     device.shell (f"input tap {xStartPVPMatchButton} {yStartPVPMatchButton}") #  PVP Match - Play Button

        #     xAvoidAction = int ((coords['btnAvoidAction']['x']) * (phoneResolution['width']))
        #     yAvoidAction = int ((coords['btnAvoidAction']['y']) * (phoneResolution['height']))
        #     device.shell (f"input tap {xAvoidAction} {yAvoidAction}") # Avoid "Go Anyway"
        ...
        device.shell(
            f"input touchscreen swipe {xStartSwipeToNextPage} {yValueSwipeToNextPage} {xEndSwipeToNextPage} {yValueSwipeToNextPage} {swipeToNextPageDuration}"
        )  # Swipe to next heroes page
        time.sleep(1.5)
    ...
    yStartPVPMatchButton = int((coords["btnPlayPVPMatch"]["y"]) * (phoneResolution["height"]))
    device.shell(
        f"input touchscreen swipe {xStartPVPMatchButton} {yStartPVPMatchButton} {xStartPVPMatchButton} {yStartPVPMatchButton} 500"
    )  #  PVP Match - Play Button

    xAvoidAction = int((coords["btnAvoidAction"]["x"]) * (phoneResolution["width"]))
    yAvoidAction = int((coords["btnAvoidAction"]["y"]) * (phoneResolution["height"]))
    device.shell(
        f"input touchscreen swipe {xAvoidAction} {yAvoidAction} {xAvoidAction} {yAvoidAction} 500"
    )  # Avoid "Go Anyway"
    ...
    while matchTime < matchDuration:
        ...
        device.shell(
            f"input touchscreen swipe {xAvoidAction} {yAvoidAction} {xAvoidAction} {yAvoidAction} 500"
        )  # Avoid "Screen Lock" for inactivity
        now = datetime.now()
        current_time = now.strftime("%H:%M:%S")
    ...
    device.shell(
        f"input touchscreen swipe {xNextButton} {yNextButton} {xNextButton} {yNextButton} 500"
    )  # Next Button == Play PVP Button
    time.sleep(1)  # 1
    ...
    device.shell(
        f"input touchscreen swipe {xPVPFreePlay} {yPVPFreePlay} {xPVPFreePlay} {yPVPFreePlay} 500"
    )  # Free Play Button
    time.sleep(1)  # 1
    ...
    device.shell(
        f"input touchscreen swipe {xPVPCrate} {yPVPCrate} {xPVPCrate} {yPVPCrate} 500"
    )  # Claim PVP Crate Button for Winning 5 Matches
    time.sleep(1)  # 1
    ...
    while matchTime <= 2:  # 4
        ...
        device.shell(
            f"input touchscreen swipe {xPVPFreePlay} {yPVPFreePlay} {xPVPFreePlay} {yPVPFreePlay} 500"
        )  # Claim PVP Rewards Crate by tapping on Free Play
        time.sleep(0.4)  # 0.2
        matchTime += 0.5  # 0.2

    print(f"Line 246 - btnPVPFreePlay")
    device.shell(
        f"input touchscreen swipe {xPVPFreePlay} {yPVPFreePlay} {xPVPFreePlay} {yPVPFreePlay} 500"
    )  # Free Play Button
    print("")

    print(f"Line 248 - btnPlayPVPMatch")
    device.shell(
        f"input touchscreen swipe {xNextButton} {yNextButton} {xNextButton} {yNextButton} 500"
    )  # Next Button == Play PVP Button
    time.sleep(1)  # 0.5
