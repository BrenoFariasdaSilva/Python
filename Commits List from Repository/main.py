...
def get_commits_information(repo_url):
    ...
    for index, commit in enumerate(Repository(path_to_repo=repo_url).traverse_commits()):  # Iterate over the commits
        ...
        for mf in commit.modified_files:  # Iterate over modified files for this commit
            path = (
                mf.new_path if getattr(mf, "new_path", None) else getattr(mf, "old_path", None)
        ...
        commits_list.append(
            (index + 1, commit.hash, commit_date, commit.msg, files_changed_str)
        )  # Append index, hash, date, message and files changed to the list
...
def add_header_to_csv(output_csv):
    ...
    with open(output_csv, mode="w", newline="") as csv_file:  # Open the CSV file in write mode
        fieldnames = [
            ...
            "Commit Message",
            "Files Changed",
        ]  # CSV header (added Files Changed)
...
def main():
    ...
    print(
        f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Commits List from Repository{BackgroundColors.GREEN}!{Style.RESET_ALL}"
    )
    ...
    print(
        f"{BackgroundColors.GREEN}Generating commits list for the repository {BackgroundColors.CYAN}{repo_name}{BackgroundColors.GREEN} in a CSV File...{Style.RESET_ALL}"
    )
