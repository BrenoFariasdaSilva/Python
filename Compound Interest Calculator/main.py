...
CONTRIBUTION = {
    ...
    "Months": "Monthly",
    "Days": "Daily",
}  # The contribution according to the period type
...
def get_period_type():
    ...
    while period_type.capitalize() not in PERIOD_TYPES:  # While the period type is not in the list of the period types
        period_type = input(
            f"{BackgroundColors.BOLD}{BackgroundColors.GREEN}Please enter the type of the period {BackgroundColors.CYAN}({STR_PERIOD_TYPES}){BackgroundColors.GREEN}: {Style.RESET_ALL}"
        )
...
def get_number_of_periods():
    ...
    while number_of_periods < 0:  # While the number of the periods is less than or equal to 0
        number_of_periods = int(
            input(
                f"{BackgroundColors.BOLD}{BackgroundColors.GREEN}Please enter the number of the periods {BackgroundColors.CYAN}(Int){BackgroundColors.GREEN}: {Style.RESET_ALL}"
            )
        )
...
def get_initial_amount():
    ...
    while initial_amount < 0:  # While the initial amount is less than or equal to 0
        initial_amount = float(
            input(
                f"{BackgroundColors.BOLD}{BackgroundColors.GREEN}Please enter the initial amount {BackgroundColors.CYAN}(Int or Float){BackgroundColors.GREEN}: {Style.RESET_ALL}"
            )
        )
...
def get_regular_contribution(period_type):
    ...
    while regular_contribution < 0:  # While the regular contribution is less than or equal to 0
        regular_contribution = float(
            input(
                f"{BackgroundColors.BOLD}{BackgroundColors.GREEN}Please enter the {CONTRIBUTION[period_type]} Contribution {BackgroundColors.CYAN}(Int or Float){BackgroundColors.GREEN}: {Style.RESET_ALL}"
            )
        )
...
def get_progressive_contribution_rate():
    ...
    while progressive_contribution < 0.00 or progressive_contribution > 1.00:
        progressive_contribution = float(
            input(
                f"{BackgroundColors.BOLD}{BackgroundColors.GREEN}Please enter the Progressive Contribution Rate{BackgroundColors.CYAN} (0.0 to 1.0){BackgroundColors.GREEN}: {Style.RESET_ALL}"
            )
        )
...
def get_interest_rate(period_type):
    ...
    while interest_rate < 0.0 or interest_rate > 1.0:  # While the interest rate is less than 0.0 or greater than 1.0
        interest_rate = float(
            input(
                f"{BackgroundColors.BOLD}{BackgroundColors.GREEN}Please enter the {period_type} Interest Rate {BackgroundColors.CYAN}(0.0 to 1.0){BackgroundColors.GREEN}: {Style.RESET_ALL}"
            )
        )
...
def calculate_compound_interest(
    number_of_periods, initial_amount, regular_contribution, progressive_contribution_rate, interest_rate
):
    ...
    for period in range(1, number_of_periods + 1):  # For each period
        current_contribution = regular_contribution * (1 + progressive_contribution_rate) ** (
            period - 1
...
def main():
    print(
        f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Compound Interest Calculator {BackgroundColors.GREEN}Algorithm!{Style.RESET_ALL}",
        end="\n\n",
    ...
    progressive_contribution_rate = (
        get_progressive_contribution_rate()
    )  # Get the progressive contribution from the user
    interest_rate = get_interest_rate(period_type)  # Get the interest rate from the user

    total_amounts, periods = calculate_compound_interest(
        number_of_periods, initial_amount, regular_contribution, progressive_contribution_rate, interest_rate
    )  # Calculate the compound interest

    money_invested = np.array(
        [initial_amount + regular_contribution * period for period in range(number_of_periods + 1)]
    )  # Calculate the money invested
    ...
    profits = [
        float(total_amounts[-1]) - money_invested[-1],
        ((float(total_amounts[-1]) - money_invested[-1]) / float(total_amounts[-1]) * 100),
    ]
    ...
    print(
        f"{BackgroundColors.BOLD}{BackgroundColors.GREEN}Profit: {BackgroundColors.CYAN}{profits[0]:.2f}{BackgroundColors.GREEN} R$, representing {BackgroundColors.CYAN}{profits[1]:.2f}%{BackgroundColors.GREEN} of the Money Invested.{Style.RESET_ALL}"
    )  # Output the profit message
