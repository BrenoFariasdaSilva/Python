...
def are_videos_similar(video_path1, video_path2):
    ...
    try:
        ...
        for frame_count in tqdm(
            range(total_frames), desc=f"{BackgroundColors.GREEN}Comparing frames{Style.RESET_ALL}", unit="frame"
        ):
        ...
        print(
            f"{BackgroundColors.BOLD}{BackgroundColors.GREEN}The similarity ratio is {BackgroundColors.CYAN}{similarity_ratio}{Style.RESET_ALL}"
        )
...
def main():
    print(
        f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Video Similarity Analyzer{BackgroundColors.GREEN}!{Style.RESET_ALL}"
    ...
    if not file_exists(first_video):  # If the first file does not exist
        print(
            f"{BackgroundColors.RED}The first file {BackgroundColors.CYAN}{first_video}{BackgroundColors.RED} does not exist. Please try again.{Style.RESET_ALL}"
        )
        return
    if not file_exists(second_video):  # If the second file does not exist
        print(
            f"{BackgroundColors.RED}The second file {BackgroundColors.CYAN}{second_video}{BackgroundColors.RED} does not exist. Please try again.{Style.RESET_ALL}"
        )
        return

    if are_videos_similar(first_video, second_video):  # If the two videos are similar
        print(
            f"{BackgroundColors.BOLD}{BackgroundColors.GREEN}The {BackgroundColors.CYAN}{first_video}{BackgroundColors.GREEN} and {BackgroundColors.CYAN}{second_video}{BackgroundColors.GREEN} videos are similar.{Style.RESET_ALL}"
        )
    else:  # If the two videos are not similar
        print(
            f"{BackgroundColors.BOLD}{BackgroundColors.RED}The {BackgroundColors.CYAN}{first_video}{BackgroundColors.RED} and {BackgroundColors.CYAN}{second_video}{BackgroundColors.RED} videos are not similar.{Style.RESET_ALL}"
        )
