...
def get_srt_file(base_name):
   ...
   verbose_output(f"{BackgroundColors.GREEN}Searching for subtitle file for: {BackgroundColors.CYAN}{base_name}{Style.RESET_ALL}")
   ...
   for root, _, files in os.walk(base_dir): # Walk through all subdirs
      for file in files: # For each file in the directory
         if file.startswith(base_no_ext) and file.endswith(".srt"): # If the file matches the base name and has .srt extension
...
def adjust_subtitle_timing(srt_file, offset_ms):
   ...
   verbose_output(f"{BackgroundColors.GREEN}Adjusting subtitle timing for: {BackgroundColors.CYAN}{srt_file}{Style.RESET_ALL}")
   ...
   try: # Try to adjust the subtitle timing
      ...
      if ext != ".srt": # If the file extension is not .srt
         print(f"{BackgroundColors.YELLOW}Skipping unsupported file: {BackgroundColors.CYAN}{srt_file}{Style.RESET_ALL}")
      ...
      adjusted_path = srt_file if REPLACE_EXISTING_SRT else os.path.splitext(srt_file)[0] + f"_offset{ext}" # Determine the adjusted file path
      subs.save(adjusted_path, format_="srt", encoding="utf-8") # Save the adjusted subtitle file

      verbose_output(f"{BackgroundColors.GREEN}Adjusted subtitle saved as: {BackgroundColors.CYAN}{adjusted_path}{Style.RESET_ALL}")
...
def process_directory(directory, offset_ms):
   ...
   srt_files = [os.path.join(directory_abs, f) for f in os.listdir(directory_abs) if f.lower().endswith(".srt")] # List of .srt files in the directory

   for srt_file in tqdm(srt_files, desc=f"{BackgroundColors.GREEN}Adjusting subtitles in {BackgroundColors.CYAN}{os.path.basename(directory_abs)}", unit="file"): # For each .srt file in the directory with a progress bar
...
def main():
   ...
   print(f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Subtitle Offset Adjuster{BackgroundColors.GREEN}!{Style.RESET_ALL}\n")
   ...
   if not dirs: # If no directories were found
      print(f"{BackgroundColors.RED}No directories found in {BackgroundColors.CYAN}Input{BackgroundColors.RED}. Please add .srt files to adjust.{Style.RESET_ALL}")
      return # Exit the program

   offset_ms = parse_offset(OFFSET) # Parse the offset string into milliseconds
   verbose_output(f"{BackgroundColors.GREEN}Applying offset of {BackgroundColors.CYAN}{OFFSET}{BackgroundColors.GREEN} ({offset_ms} ms){Style.RESET_ALL}")
