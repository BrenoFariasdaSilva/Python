...
def validate_inputs(src_dir=SRC_DIR, dst_dir=DST_DIR):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Validating the source and destination directories{Style.RESET_ALL}"
    ...
    if not verify_filepath_exists(src_dir):  # Verify if the source directory exists
        print(
            f"{BackgroundColors.RED}Source directory does not exist: {src_dir}{Style.RESET_ALL}"
    ...
    if not verify_filepath_exists(dst_dir):  # Verify if the destination directory exists
        ...
        try:  # Try to create the destination directory
            ...
            print(
                f"{BackgroundColors.GREEN}Destination directory created: {dst_dir}{Style.RESET_ALL}"
            )  # Output the success message
        except Exception as e:  # If an error occurs (e.g., permission denied)
            print(
                f"{BackgroundColors.RED}Error creating destination directory: {e}{Style.RESET_ALL}"
            )  # Output the error message
            valid_inputs = False  # Set valid inputs to False
...
def get_platform_ignore_dirs():
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Getting the ignored directories for the current platform: {BackgroundColors.CYAN}{platform.system()}{Style.RESET_ALL}"
    )  # Output the verbose message
...
def get_files_to_copy(src_dir):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Getting all files to copy from the source directory: {BackgroundColors.CYAN}{src_dir}{Style.RESET_ALL}"
    )  # Output the verbose message

    all_paths = []  # List to store all file and empty directory paths
    ignore_paths = [
        os.path.normpath(os.path.join(src_dir, path)) for path in get_platform_ignore_dirs()
    ]  # Get the ignored directories for the current platform
    ...
    for root, dirs, files in os.walk(src_dir):  # Walk through the source directory
        ...
        if any(
            root_normalized.startswith(ignore_dir) for ignore_dir in ignore_paths
        ):  # Verify if the current directory is in the ignored directories
            verbose_output(
                f"{BackgroundColors.YELLOW}Skipping ignored directory: {BackgroundColors.CYAN}{root}{Style.RESET_ALL}"
            )
...
def get_file_size(file_path):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Getting the size of the file: {BackgroundColors.CYAN}{file_path}{Style.RESET_ALL}"
    )  # Output the verbose message
...
def copy_file(src_file, dst_file):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Copying file from {BackgroundColors.CYAN}{src_file}{BackgroundColors.GREEN} to {BackgroundColors.CYAN}{dst_file}{Style.RESET_ALL}"
    )  # Output the verbose message
...
def calculate_speed(file_size, elapsed_time):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Calculating the speed of the file copy: {BackgroundColors.CYAN}{file_size}{BackgroundColors.GREEN} bytes in {BackgroundColors.CYAN}{elapsed_time}{BackgroundColors.GREEN} seconds{Style.RESET_ALL}"
    )  # Output the verbose message
...
def list_top_level_dirs(base_dir):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Listing top-level directories in the base directory: {BackgroundColors.CYAN}{base_dir}{Style.RESET_ALL}"
    )  # Output the verbose message

    ignore_dirs = [
        name.lower() for name in get_platform_ignore_dirs()
    ]  # Get the ignored directories for the current platform
    try:  # Try to list the directories
        ...
        print(
            f"{BackgroundColors.RED}Error reading directories in {base_dir}: {e}{Style.RESET_ALL}"
        )  # Output the error message
...
def get_dir_size(directory):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Calculating the size of the directory: {BackgroundColors.CYAN}{directory}{Style.RESET_ALL}"
    )  # Output the verbose message
    ...
    for dirpath, dirnames, filenames in os.walk(directory):  # Walk through the directory
        ...
        if any(
            dirpath_normalized.startswith(ignore_dir) for ignore_dir in ignore_paths
        ):  # Verify if the current directory is in the ignored directories
            continue  # Skip this directory if it contains an ignored directory
...
def print_top_level_dirs_to_copy(base_dir):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Printing top-level directories to copy from: {BackgroundColors.CYAN}{base_dir}{Style.RESET_ALL}"
    )  # Output the verbose message
    ...
    if top_dirs:  # If there are directories to copy
        print(
            f"{BackgroundColors.BOLD}{BackgroundColors.GREEN}Directories to be copied:{Style.RESET_ALL}"
        )  # Output the header
        for dir_name in top_dirs:  # Iterate through all top-level directories
            ...
            print(
                f"{BackgroundColors.CYAN}- {dir_name} ({format_size(dir_size)}){Style.RESET_ALL}"
            )  # Output the directory name and size

        print(
            f"{BackgroundColors.GREEN}Total size: {BackgroundColors.CYAN}{format_size(total_size)}{Style.RESET_ALL}\n"
        )  # Output the total size of all directories
    else:  # If there are no directories to copy
        print(
            f"\n{BackgroundColors.RED}No valid directories found to copy.{Style.RESET_ALL}"
        )  # Output the error message
...
def copy_and_track_files(src_dir, dst_dir):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Copying files from {BackgroundColors.CYAN}{src_dir}{BackgroundColors.GREEN} to {BackgroundColors.CYAN}{dst_dir}{Style.RESET_ALL}"
    )  # Output the verbose message
    ...
    with tqdm(
        total=len(all_files), unit="file", desc=f"{BackgroundColors.GREEN}Copying Files{Style.RESET_ALL}"
    ) as pbar:  # Initialize the progress bar
        for src_file in all_files:  # Iterate through all files to copy
            ...
            top_dir = (
                relative_path.split(os.sep)[0] if os.sep in relative_path else ""
            ...
            if top_dir != current_top_dir:  # If the top-level directory has changed
                ...
                pbar.set_description(
                    f"{BackgroundColors.GREEN}Copying: {BackgroundColors.CYAN}{current_top_dir or 'root'}{Style.RESET_ALL}"
            ...
            try:  # Try to copy the file
                ...
                pbar.set_postfix(
                    {"Speed": format_size(speed_Bps) + "/s"}
                )  # Update the progress bar postfix with the formatted speed
                pbar.update(1)  # Update the progress bar
            except Exception as e:  # If an error occurs (e.g., permission denied)
                print(
                    f"{BackgroundColors.RED}Error copying {src_file}: {e}{Style.RESET_ALL}"
                )  # Output the error message
    ...
    return (
        ...
        avg_speed_Bps,
        file_speeds,
    )  # Return the total time, total bytes copied, average speed, and file speeds
...
def format_duration(minutes):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Formatting duration from {BackgroundColors.CYAN}{minutes}{BackgroundColors.GREEN} minutes to a human-readable string{Style.RESET_ALL}"
    )  # Output the verbose message
    ...
    if days:
        parts.append(f"{days}d")  # If there are days, add them to the list
    if hours:
        parts.append(f"{hours}h")  # If there are hours, add them to the list
    if mins:
        parts.append(f"{mins}m")  # If there are minutes, add them to the list
    if not parts or seconds:
        parts.append(f"{seconds}s")  # If there are no parts or there are seconds, add them to the list
...
def format_size(bytes_size):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Formatting size from {BackgroundColors.CYAN}{bytes_size}{BackgroundColors.GREEN} bytes to a human-readable string{Style.RESET_ALL}"
    )  # Output the verbose message
...
def print_summary(total_time, total_bytes, avg_speed):
    ...
    print(
        f"{BackgroundColors.GREEN}- Total Time:{BackgroundColors.CYAN} {format_duration(total_time)}{Style.RESET_ALL}"
    )  # Output the total time taken for the copy operation
    print(
        f"{BackgroundColors.GREEN}- Total Data:{Style.RESET_ALL} {format_size(total_bytes)}"
    )  # Output the total data copied
    print(
        f"{BackgroundColors.CYAN}- Average Write Speed: {BackgroundColors.GREEN}{format_size(avg_speed)}{Style.RESET_ALL}"
    )  # Output the average speed of the copy operation
...
def ensure_output_directory(path="./Output"):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Ensuring the output directory exists at: {BackgroundColors.CYAN}{path}{Style.RESET_ALL}"
    )  # Output the verbose message
...
def generate_copy_speed_plot(file_speeds, average_speed):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Plotting the file copy speed over time{Style.RESET_ALL}"
    )  # Output the verbose message
    ...
    ax.axhline(
        average_speed, color="red", linestyle="--", label=f"Average: {formatted_average_speed}/s"
    )  # Plot the average speed using human-readable format
...
def save_copy_speed_plot(fig):
    ...
    verbose_output(
        f"{BackgroundColors.GREEN}Saving the plot to {BackgroundColors.CYAN}./Output{Style.RESET_ALL}"
    )  # Output the verbose message
...
def main():
    ...
    print(
        f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Copying files from {BackgroundColors.CYAN}{SRC_DIR}{BackgroundColors.GREEN} to {BackgroundColors.CYAN}{DST_DIR}{Style.RESET_ALL}\n"
    )
    ...
    total_time, total_bytes, avg_speed, speeds = copy_and_track_files(
        SRC_DIR, DST_DIR
    )  # Copy files and track the speed
    print_summary(total_time, total_bytes, avg_speed)  # Print the summary of the copy operation

    fig = (
        generate_copy_speed_plot(speeds, avg_speed) if RUN_FUNCTIONS["Plotting"] or RUN_FUNCTIONS["Save Plot"] else None
    )  # Generate the plot if the RUN_FUNCTIONS["Plotting"] or RUN_FUNCTIONS["Save Plot"] is set to True
    (
        save_copy_speed_plot(fig) if RUN_FUNCTIONS["Save Plot"] else None
    )  # Save the plot if the RUN_FUNCTIONS["Save Plot"] is set to True
    (
        show_copy_speed_plot(fig) if RUN_FUNCTIONS["Plotting"] else None
    )  # Show the plot if the RUN_FUNCTIONS["Plotting"] is set to True

    print(f"\n{BackgroundColors.BOLD}{BackgroundColors.GREEN}Program finished.{Style.RESET_ALL}")

    (
        atexit.register(play_sound) if RUN_FUNCTIONS["Play Sound"] else None
    )  # Register the play_sound function to be called at exit
